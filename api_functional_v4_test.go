...
// Test putObjectMultipartFromReadAt with a fake progress bar.
func TestPutObjectMultipartFromReadAt(t *testing.T) {
	...
	progressBuf := make([]byte, dataSize)
	// Create a blank reader to simulate progress updating.
	progress := bytes.NewReader(progressBuf)

	// Save the data
	objectName := randString(60, rand.NewSource(time.Now().UnixNano()), "")
	// Attempt to upload the data.
	n, err := c.putObjectMultipartFromReadAt(bucketName, objectName, bytes.NewReader(buf), int64(len(buf)), "application/octet-stream", progress)
	if err != nil {
		t.Fatal("Error:", err, bucketName, objectName)
	}
	...
	// Compare the number of bytes read to the progress bar to those uploaded.
	progressBytes, err := progress.Read(progressBuf)
	if err != nil {
		t.Fatal("Error:", err)
	}
	if progressBytes != len(buf) {
		t.Fatalf("Error: progress bar bytes not updated correctly, wanted %v, got %v", len(buf), progressBytes)
	}
